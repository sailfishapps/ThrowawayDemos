...
function getTemplates(area_id) {
    ...
    //AFTER MANY TESTS, NOW SOLVED
    // ? MEANS FILTER IN
    // Java style equals == or ==== are required!!!!

    //var query = "$.landed.area[(@.id == " + area_id + ")].template[*]"

    // return all those where area id is 0, should return nothing
    //var query = "$.landed.area[?(@.id == 0)].template[*]" // returns nothing -->  OK

    // return all all those where area id is 1, should return 11, 12
    //var query = "$.landed.area[?(@.id == 1)].template[*]" // returns 11, 12 --> OK

    // return all all those where area id is 2, should return 21, 22
    //var query = "$.landed.area[?(@.id == 2)].template[*]" // returns 21, 22 --> OK

    // return all those where area id is 3, should return 31, 32
    //var query = "$.landed.area[?(@.id == 3)].template[*]" // returns 31, 32 --> OK

    // return all those where the area id is NOT 0, should return 11, 12, 21, 22, 31, 32
    //var query = "$.landed.area[?(@.id != 0)].template[*]" // returns 11, 12, 21, 22, 31, 32 --> OK

    // return all those where the area id is NOT 1, should return 21, 22, 31, 32
    //var query = "$.landed.area[?(@.id != 1)].template[*]" // returns 21, 22, 31, 32 --> OK

    // return all those where the area id is NOT 2, should return 11, 22, 31, 32
    //var query = "$.landed.area[?(@.id != 2)].template[*]" // returns 11, 22, 31, 32 --> OK

    // return all those where the area id is NOT 3, should return 11, 12, 21, 22
